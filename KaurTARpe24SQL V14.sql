...
from Employees
order by Id
-- Order by ei tohi kasutada view
...
-- peale iga töötaja tahame teaa saada töötaja ID-d, päeva ning aega (millal sisestati), kõik andmed tulevad EmployeeAudit tabelisse

create trigger trEmployeeForInsert on Employees
for insert
as begin
Declare @Id int
select @Id = Id from inserted
insert into EmployeeAudit
values ('New employee with Id = ' + CAST(@Id as nvarchar(20)) + ' is aded at ' + CAST(getdate() as nvarchar(20)))
end
...
update Employees set FirstName = 'test123', Salary = 4000, MiddleName = 'test456' where Id = 10
...
as begin
	...
	from Department
	join inserted on inserted.DepartmentName = Department.DepartmentName
...
-- esimene parameeter on veateate sisu, teine n veataseme nr
-- (nr 16 tähendab üldiseidvigu),
-- kolmas on olek
...
set Name = 'Johny', DepartmentName = 'IT' where Id = 1
-- ei saa uuendada andmeid kuna mitu tabelit on sellest mõjutatud


update vEmployeeDetails
set DepartmentName = 'IT' where Id = 1
-- nüüd saab uuendada kuna ainult ühes tabelis tahame muuta andmeid
...
-- näitab ära osakonnad kus töötajaid 2tk või rohkem
...
-- Muudame Id 1 all oleva isiku sugu Female peale
...
-- kasutame joini ja muudame ühes tabels andmeid
...
update EmployeesByDepartment set DepartmentName = 'HR' where Id = 1
...
-- korduv CTE
-- CTE, Mis iseendale vitab, kutsutakse korduvaks CTE-ks
-- kui tahad andmeid näidata hierariliselt
...
sp_rename 'Employee.DepartmentId', 'ManagerId'
...
as
(
	select Employee.Id, Name, ManagerId, 1 from Employee
	where ManagerId is null
	union all
	select Employee.Id, Employee.Name, Employee.ManagerId, EmployeesCTE.[Level] + 1
	from Employee
	join EmployeesCTE
...
insert into ProductsSales values('Tom', 'UK', 200)
insert into ProductsSales values('John', 'US', 180)
insert into ProductsSales values('John', 'UK', 260)
insert into ProductsSales values('David', 'India', 450)
insert into ProductsSales values('Tom', 'India', 350)
insert into ProductsSales values('David', 'US', 200)
insert into ProductsSales values('Tom', 'US', 130)
insert into ProductsSales values('John', 'India', 540)
insert into ProductsSales values('John', 'UK', 120)
insert into ProductsSales values('David', 'UK', 220)
insert into ProductsSales values('John', 'UK', 420)
insert into ProductsSales values('David', 'US', 320)
insert into ProductsSales values('Tom', 'US', 340)
insert into ProductsSales values('Tom', 'UK', 660)
insert into ProductsSales values('John', 'India', 430)
insert into ProductsSales values('David', 'India', 230)
insert into ProductsSales values('David', 'India', 280)
insert into ProductsSales values('Tom', 'UK', 480)
insert into ProductsSales values('John', 'UK', 360)
insert into ProductsSales values('David', 'UK', 140)
...
select SalesAgent, India, US, UK from ProductSalesWithId pivot
(
	sum(SalesAmount) for SalesCountry in ([India], [US], [UK])
)
as PivotTable
-- põhjuseks, miks näitab NULL-e, et on olemas Id veerg, mida võetakse arvesse pööramise ja grupeerimise järgi
...
from
(
	select Id, SalesAgent from ProductSalesWithId
)
as SourceTable
unpivot
(
	SalesAmount for SalesCountry in (SalesAgent)
)
as PivotTable
...
insert into MailingAddress values (1, 101, '#10', 'King Street', 'London', 'CR27DW') 
...
as begin
	begin try
		begin transaction
			update MailingAddress set City = 'LONDON' where MailingAddress.Id = 1 and EmployeeNumber = 101
			update PhysicalAddress set City = 'LONDON' where PhysicalAddress.Id = 1 and EmployeeNumber = 101
		commit transaction
	end try
	begin catch
		rollback tran
...
as begin
	begin try
		begin transaction
			update MailingAddress set City = 'LONDON 12' where MailingAddress.Id = 1 and EmployeeNumber = 101
			update PhysicalAddress set City = 'LONDON LONDON' where PhysicalAddress.Id = 1 and EmployeeNumber = 101
		commit transaction
	end try
	begin catch
		rollback tran
...
--- Atomic - kõik tehingud transactions on kas edukalt täidetud või need
...
--- Durable - ui muudatus on tehtud, siis see on püsiv. Kui süsteemiviga või
...
select Name,
(select sum(QuantitySold) from ProductSales where ProductId = Product.Id) as [Total Quantity]
from Product
order by Name

select Product.Name, sum(QuantitySold) as [Total Quantity] from Product
left join ProductSales
on Product.Id = ProductSales.ProductId
Group BY name
order by name

--- sub queryt saab subquery sisse panna
...
select Product.Id, Name, Description, ProductId, UnitPrice, QuantitySold from Product
inner join ProductSales on Product.Id = ProductSales.Id

-- õp variant
select distinct Product.Id, Name, Description, ProductId, UnitPrice, QuantitySold from Product
inner join ProductSales on Product.Id = ProductSales.ProductId
...
-- 7 606 798 rida 39 sekundiga

-- kasutame left joini

select distinct Product.Id, Name, Description, ProductId, UnitPrice, QuantitySold from Product
left join ProductSales on Product.Id = ProductSales.ProductId where ProductSales.ProductId is NULL
...
-- open aaldusega täidab select avaldust
...
-- kasutame joini
-- kontrollime, kas product 55, 65 ja 1000 on uuendatud?

select * from Product
inner join ProductSales on Product.Id = ProductSales.ProductId
where Product.Name = 'Product - 55' OR Product.Name = 'Product - 65' or Product.Name = 'Product - 1000'
...
select * from Product join ProductSales on Product.Id = ProductSales.ProductId where(Name = 'Product - 55' or Name = 'Product - 65' or Name = 'Product - 1000'
)
...
begin
	...
	print 'Table EmployeeTest deleted'
...
if not exists(select * from INFORMATION_SCHEMA.COLUMNS where COLUMN_NAME = 'Email1234' and TABLE_NAME = 'EmployeeTest' and TABLE_SCHEMA = 'dbo')
begin
	alter table EmployeeTest
...
begin
	print 'Column Email already exists in Employee'
end

if COL_LENGTH('Employee', 'Email1234') is not null
begin
...
-- 2. kui read on StudentSource tabelis olemas, aga neid ei ole StudentTarget-s, siis puuduolevad read sisestatakse
-- 3. kui read on olemas StudentTarget-s, aga mitte StudentSource-s, Siis StudentTarget tabelis kustutatakse ära
...
-- ühe tööüksusena. Kas kõik käsud õnnestuvad või mitte midagi
-- Kui üks tehing selest ebaõnnestub
...
-- probleemi lahndamiseks pakub MSSQL server erinevaid tehinguisolatsiooni tasemeid,
...
begin tran
update Inventory
set ItemsInStock= ItemsInStock-2 where Id = 1
...
-- ja teine transaction uuendab neid andmeid esimese
-- ning teise käsu vahel esimese transactioni jooksutamise ajal
...
-- transaction 2
-- uuendab Invetory tabelis 5 telefoni peale
Update Inventory
set ItemsInStock = 5 where Id = 1

-- uuendasin andmeid, aga esimeses servis
...
--- Kuidas SQL server tuvastab deadlokci?
-- lukustatakse serveri lõim, mis töötab vaikimisi iga 5 sek järel
-- et tuvastada ummikuid. Kui leiab deadlocki, sis langeb
